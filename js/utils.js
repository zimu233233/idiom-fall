...
const CFG = {
  ...
  // 文思直通（墨池蓄满触发）：双栏二选一速答环节
  RUSH_NEED: 16,          // 墨池容量：选对 16 字蓄满
  RUSH_ENERGY_WRONG: 0.5, // 正常玩法选错字扣减的墨池能量（下限 0）
  RUSH_DROP: 480,         // 直通中每字的坠落距离（双栏生成在当前人物下方的距离）
  RUSH_CHAR_SEC: 2,      // 全对时每字的标准耗时；能量按 8×该值（两条成语）线性耗尽
  RUSH_BEAT: 0.35,        // 答对一字后的间歇
  RUSH_BOUNCE_WAIT: 0.3,  // 答错反弹后到自动闪现另一栏的等待
  RUSH_PLAT_W: 210,       // 双栏单块平台宽（默认半幅，两块无缝铺满画布横面）
  RUSH_HP_MULT: 0.5,      // 直通中生命扣除倍率（错字惩罚与时间消耗同乘）
};
